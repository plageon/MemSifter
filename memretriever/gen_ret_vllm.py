...
import requests
import yaml
from anytree import PreOrderIter, Node, RenderTree
...
def process_haystack_sessions(tokenizer: Any, entry: Dict, max_content_length: int = 65536, output_tag=False):
    ...
    for session_idx in range(len(haystack_sessions)):
        ...
        session_prefix = f'<session{session_idx}>\n'
        ...
        for turn in session:
            ...
            if turn["role"] == "user":
                ...
                turn_input_text = f'<user{user_id}>{turn["content"]}</user>\n'
            elif turn["role"] == "assistant":
                turn_name = f"assistant_{assistant_id}"
                assistant_id += 1
                turn_type = NodeType.ASSISTANT
                turn_input_text = f'<assistant{assistant_id}>{turn["content"]}</assistant>\n'
            else:
                loguru.logger.warning(f"Unknown role {turn['role']} in session {session_idx}, skipping this turn.")
                continue

            # 计算当前轮次的token数量
            turn_input_ids = tokenizer.encode(turn_input_text, add_special_tokens=False)
            if output_tag:
                turn_output_ids = tokenizer.encode(
                    f"{session_prefix}{turn_input_text}",
                    add_special_tokens=False)
            else:
                turn_output_ids = tokenizer.encode(turn["content"], add_special_tokens=False)
    ...
    for turn in kept_turns:
        ...
        # 更新会话文本和历史
        current_session_node.text += turn['turn_input_text']
        history += turn['turn_input_text']
        all_turns.append(turn_node)
...
def session_node_probabilities(his_root_node, token_root, all_turn_token_ids, haystack_sessions, haystack_session_ids):
    """计算会话节点和回合节点的概率

    Args:
        his_root_node: 历史根节点，包含所有会话节点
        token_root: 令牌树的根节点
        all_turn_token_ids: 所有回合的令牌ID列表

    说明：
        此函数会修改节点的prob属性，是一个原地操作
    """
    # 遍历所有会话节点
    for session_node in his_root_node.children:
        ...
        for turn_node in session_node.children:
            ...
            loop_count = 0
            while len(curr_token_node.children) and len(curr_token_node.input_ids) < len(turn_node.output_ids):
                # find the child node with this token id
                # logger.info(f"current token node: {curr_token_node.input_ids}")
                found_child = False
                loop_count += 1
                for child_node in curr_token_node.children:
                    # logger.debug(f"child node: {child_node.input_ids}")
                    token_length = min(len(child_node.input_ids), len(turn_node.output_ids))
                    if child_node.input_ids[:token_length] == turn_node.output_ids[:token_length]:
                ...
                if not found_child:
                    ...
                    for all_turn_token_id in all_turn_token_ids:
                        if turn_node.output_ids == all_turn_token_id[:len(turn_node.output_ids)]:
                            break
        ...
        for session_node in his_root_node.children:
            if session_node.session_id == haystack_session_ids[session_idx]:
                ...
                for turn_idx in range(len(haystack_sessions[session_idx])):
                    for turn_node in session_node.children:
                        if turn_node.embedding_hash_id == haystack_sessions[session_idx][turn_idx][
                            'embedding_hash_id']:
...
def turn_node_probabilities(his_root_node, token_root, all_turns, haystack_sessions, haystack_session_ids):
    # calculate turn node probabilities
    for turn_node in all_turns:
        # logger.info(f"turn node: {turn_node.output_ids}")
        curr_token_node = token_root
        turn_node.prob = np.float32(1.0)
        loop_count = 0
        while len(curr_token_node.children) and len(curr_token_node.input_ids) < len(turn_node.input_ids):
            ...
            loop_count += 1
            for child_node in curr_token_node.children:
                # logger.debug(f"child node: {child_node.output_ids}")
                ...
                for turn_idx in range(len(haystack_sessions[session_idx])):
                    for turn_node in session_node.children:
                        if turn_node.embedding_hash_id == haystack_sessions[session_idx][turn_idx]['embedding_hash_id']:
                            if turn_node.prob > 0.0:
                                logger.info(f"turn node: {turn_node.name}, prob: {turn_node.prob}")
                            haystack_sessions[session_idx][turn_idx]['gen_prob'] = float(turn_node.prob)
                            found_child = True
                            break
                    if not found_child:
...
async def process_single_entry(
    ...
    # </session>
    try:
        processed_haystack = process_haystack_sessions(tokenizer, entry, max_content_length, output_tag)
        history = processed_haystack["history"]
        all_turns = processed_haystack["all_turns"]
        ...
        all_turn_token_ids = [turn.output_ids for turn in all_turns]
        ...
        # pbar.close()
        # calculate turn node probabilities
        # turn_node_probabilities(his_root_node, token_root, all_turns, haystack_sessions, haystack_session_ids)
        session_node_probabilities(his_root_node, token_root, all_turn_token_ids, haystack_sessions, haystack_session_ids)
...
async def main(
        api_request_semaphore: asyncio.Semaphore,
        dataset_name: str,
        output_dir: str,
        split: str,
        embed_model: str,
    ...
    elif max_content_length == 8192:
        output_file = output_file.replace(".jsonl", f"_8k.jsonl")
    else:
        logger.error(f"Unknown max_content_length: {max_content_length}")

    if not os.path.exists(f"./data/{model_tag}"):
        os.makedirs(f"./data/{model_tag}", exist_ok=True)
    ...
    import sys
    sys.path.append("./")
    from baseline_eval.eval_retrieval import eval_prob_data
...
if __name__ == '__main__':
    ...
    arg_parser.add_argument("--max-content-length", type=int, default=32768, help="Maximum content length")
    ...
    arg_parser.add_argument("--output-tag", action="store_true", help="Output tag in model output")
